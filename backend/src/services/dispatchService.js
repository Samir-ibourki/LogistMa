 ...
 */
export async function findBestDriver(parcel) {
...
export async function dispatchParcel(parcelId) {
    ...
    const route = calculateRoute(Number(parcel.pickupLat), Number(parcel.pickupLng), Number(parcel.deliveryLat), Number(parcel.deliveryLng));
 ...
 */
export async function markAsPickedUp(deliveryId) {
    ...
    await Parcel.update({ status: 'picked_up' }, { where: { id: delivery.parcelId } });
    return { success: true, message: 'Parcel picked up' };
}
/**
 * Mark a delivery as completed
 */
export async function markAsDelivered(deliveryId) {
    const delivery = await Delivery.findByPk(deliveryId);
    if (!delivery) {
        return { success: false, message: 'Delivery not found' };
    }
    ...
    await Parcel.update({ status: 'delivered' }, { where: { id: delivery.parcelId } });
    // 3. Free the driver
    await Driver.update({ status: 'available' }, { where: { id: delivery.driverId } });
 ...
 */
export async function markAsFailed(deliveryId, reason) {
    const delivery = await Delivery.findByPk(deliveryId);
    if (!delivery) {
        return { success: false, message: 'Delivery not found' };
    }
    ...
    await Parcel.update({ status: 'pending', driverId: null }, { where: { id: delivery.parcelId } });
    // 3. Free the driver
    await Driver.update({ status: 'available' }, { where: { id: delivery.driverId } });
