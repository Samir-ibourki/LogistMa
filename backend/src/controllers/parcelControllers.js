...
export const createParcel = async (req, res, next) => {
    try {
        const { pickupAddress, pickupLat, pickupLng, deliveryAddress, deliveryLat, deliveryLng, weight, zoneId, } = req.body;
...
export const getParcelByTrackingCode = async (req, res, next) => {
    try {
 ...
 */
export const dispatchParcel = async (req, res, next) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({
